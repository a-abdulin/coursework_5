...
class Arena(metaclass=BaseSingleton):
    ...
    def _check_players_hp(self):
        ...
        if self.player.hp > 0 and self.enemy.hp > 0:
            ...
            self.battle_result = f"{self.player.name} проиграл. Противник победил!"
        else:
            self.battle_result = f"{self.enemy.name} проиграл. Игрок победил!"
    ...
    def _stamina_regeneration(self):
        ...
        for unit in units:
            if (unit.stamina+unit.unit_class.stamina*self.STAMINA_PER_ROUND) < unit.unit_class.max_stamina:
